...
import sqlite3, datetime, os, threading, urllib.request, urllib.parse, json
app = Flask(__name__)
...
def send_email(to, subject, html_body):
    if not SENDGRID_API_KEY or not SENDER_EMAIL:
        print("WARNING: Email not configured — set SENDGRID_API_KEY and SENDER_EMAIL in Railway variables")
        return False
    try:
        payload = json.dumps({
            "personalizations": [{"to": [{"email": to}]}],
            "from": {"email": SENDER_EMAIL, "name": "Club 72 Golf Co."},
            "subject": subject,
...
def book():
    ...
    # Send emails in background so the booking response isn't held up
    def send_emails():
        notify_nathan(d, bid)
